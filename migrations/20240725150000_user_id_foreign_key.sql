...
DROP TRIGGER delete_user_trigger ON public.users;
DROP TRIGGER update_af_user_deleted_at_trigger ON public.users;
DROP FUNCTION public.delete_user();
DROP FUNCTION public.update_af_user_deleted_at();

-- Delete all users from public.af_user table that are not in public.users table
